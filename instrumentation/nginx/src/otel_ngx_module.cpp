...
static void OtelCaptureHeaders(nostd::shared_ptr<opentelemetry::trace::Span> span,
                               ngx_str_t keyPrefix,
                               ngx_list_t *headers,
#if (NGX_PCRE)
                               ngx_regex_t *sensitiveHeaderNames,
                               ngx_regex_t *sensitiveHeaderValues,
#endif
                               nostd::span<ngx_table_elt_t *> excludedHeaders)
{
  for (ngx_list_part_t *part = &headers->part; part != nullptr; part = part->next)
  {
    ...
    {
      if (std::find(excludedHeaders.begin(), excludedHeaders.end(), &header[i]) !=
          excludedHeaders.end())
      {
        continue;
      }
      ...
      NgxNormalizeAndCopyString((u_char *)ngx_copy(key, keyPrefix.data, keyPrefix.len),
                                header[i].key);
      ...
      span->SetAttribute({(const char *)key, keyPrefix.len + header[i].key.len},
                         nostd::span<const nostd::string_view>(&value, 1));
    }
  }
}
...
static ngx_int_t OtelGetTraceContextVar(ngx_http_request_t *req,
                                        ngx_http_variable_value_t *v,
                                        uintptr_t data);

static ngx_int_t OtelGetTraceId(ngx_http_request_t *req,
                                ngx_http_variable_value_t *v,
                                uintptr_t data);

static ngx_int_t OtelGetSpanId(ngx_http_request_t *req,
                               ngx_http_variable_value_t *v,
                               uintptr_t data);
...
#if (NGX_PCRE)
    int ovector[3];
    return locConf->ignore_paths == nullptr ||
           ngx_regex_exec(locConf->ignore_paths, &req->unparsed_uri, ovector, 0) < 0;
#else
    return true;
#endif
  }
...
TraceContext *GetTraceContext(ngx_http_request_t *req)
{
  ...
  std::unordered_map<ngx_http_request_t *, TraceContext *> *map =
      (std::unordered_map<ngx_http_request_t *, TraceContext *> *)val->data;
  if (map == nullptr)
  {
    ngx_log_error(NGX_LOG_INFO, req->connection->log, 0, "TraceContext not found");
    return nullptr;
...
{
  std::unordered_map<ngx_http_request_t *, TraceContext *> *map =
      (std::unordered_map<ngx_http_request_t *, TraceContext *> *)data;
  map->~unordered_map();
}
...
TraceContext *CreateTraceContext(ngx_http_request_t *req, ngx_http_variable_value_t *val)
{
  ...
  {
    ngx_pool_cleanup_t *cleanup = ngx_pool_cleanup_add(
        req->pool, sizeof(std::unordered_map<ngx_http_request_t *, TraceContext *>));
...
ngx_int_t StartNgxSpan(ngx_http_request_t *req)
{
  ...
  // Internal requests must be called from another location in nginx, that should already have a
  // trace. Without this check, a call would generate an extra (unrelated) span without much
  // information
  if (req->internal)
  {
    return NGX_DECLINED;
  ...
  context->request_span =
      GetTracer()->StartSpan(GetOperationName(req),
                             {
  ...
  {
    context->request_span->SetAttribute("http.user_agent",
                                        FromNgxString(req->headers_in.user_agent->value));
  }

  if (locConf->captureHeaders)
  {
    ngx_table_elt_t *excludedHeaders[] = {req->headers_in.host, req->headers_in.user_agent};
    OtelCaptureHeaders(context->request_span, ngx_string("http.request.header."),
                       &req->headers_in.headers,
#if (NGX_PCRE)
...
void AddScriptAttributes(trace::Span *span, const ngx_array_t *attributes, ngx_http_request_t *req)
{
...
{
  OtelMainConf *mainConf =
      (OtelMainConf *)ngx_http_conf_get_module_main_conf(conf, otel_ngx_module);

  ngx_str_t *values = (ngx_str_t *)conf->args->elts;
  ngx_str_t *path   = &values[1];

  if (!OtelAgentConfigLoad(std::string((const char *)path->data, path->len), conf->log,
                           &mainConf->agentConfig))
  {
    return (char *)NGX_CONF_ERROR;
  }
...
static ngx_regex_t *NgxCompileRegex(ngx_conf_t *conf, ngx_str_t pattern)
{
  ...
  {
    ngx_log_error(NGX_LOG_ERR, conf->log, 0, "illegal regex in %V: %V",
                  (ngx_str_t *)conf->args->elts, &rc.err);
    return nullptr;
  }
...
static std::unique_ptr<sdktrace::SpanProcessor> CreateProcessor(
    const OtelNgxAgentConfig *conf,
    std::unique_ptr<sdktrace::SpanExporter> exporter)
{
  ...
  auto provider  = nostd::shared_ptr<opentelemetry::trace::TracerProvider>(
      new sdktrace::TracerProvider(std::move(processor),
                                    opentelemetry::sdk::resource::Resource::Create(
                                       {{"service.name", agentConf->service.name}}),
                                    std::move(sampler)));
